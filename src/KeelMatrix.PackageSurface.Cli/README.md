...
Analysis does not load dependency assemblies, execute MSBuild or package-provided build code, crawl the global cache, or use the network after restore. XML, archive metadata, file, graph, metadata, and hashing work is bounded. Traversal-looking paths, unsafe links, malformed XML, corrupt metadata, and invalid PE metadata fail closed. The tool targets modern SDK-style `PackageReference` projects on `net8.0`; Windows, Linux, and macOS are intended platforms, with Windows verified for this candidate and the other platforms unverified.

Best-effort activation telemetry uses `KeelMatrix.Telemetry` only after a successful baseline or check. It excludes package identities, versions, asset names and paths, target names, TFM/RID values, content, hashes, diagnostics, baselines, repository identity, and feed information. Disable it with `--telemetry off` or `--no-telemetry`; failures never affect the result.
